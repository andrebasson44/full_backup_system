...
##OVERVIEW
 ...
 #DESCRIPTION:  Typical FULL (image) backup script, which syncs data - files and directories listed in ./toBackup.txt - either between a 
  ...
  #              remote host (as opposed to the remote host instigating the transfer by PUSHing the data).  The other two scenarios PUSHes data.
  #
  #				This script is originally scaled down from a version of the incremental-backup script, incremental-backup_PULL-version_v1.25.sh.
  #
 #IMPORTANT:	
  #       A. In any scenario, this script caters for AT MOST one remote host (from/to which to PULL/PUSH)	at a time.
 ...
 #VERSIONS:     based on full_backup_PUSH-and-PULL-version_v1.287.sh 
  ...
  #             v1.32   - OVERALL STATUS: works pretty well! 
   ...
   #
  #               v1.311: OVERALL STATUS: WORKS PRETTY WELL!
   #                   - CHANGES: indentation of lines only - to make code section collapsable in modern coding packages (eg. VSCode, Atom, etc.) 
  #               v1.31: OVERALL STATUS: WORKS PRETTY WELL! (1 issue uncovered from v1.3)
   ...
   #                                 >> SOLUTION WORKED!!
   #
   #                          2. elementIn () ammended - line 'exit 1' replaced with 'return 1'
   #
  #               v1.3:   OVERALL STATUS: WORKS WELL
   #                        - SOLVED:       
   #                           1. concurrency issue from v1.286
   #                           2. logging changes from v1.288
   #
  #               v1.288: OVERALL STATUS: IN PROGRESS (see below), WITH v1.286 issue pending
   #                        - IN PROGRESS:
   #                          >   introducing 'rsync_log_regexp' and 'error_log_regexp' variables (to be retrieved from config file ./full-backup.conf) 
   #                              instead of hard-coded regular expressions '.*(rsync)+.*(log)' and '.*(error)+.*(log)'.  Introduced to
   #                              correctly differentiate between different types of log files that should (and shouldn't) be deleted.
   #
   #
  #               v1.2872: OVERALL STATUS: WORKS VERY WELL, WITH v1.286 issue pending
   #                        - introduced a text file ./full-backup.conf to replace the current task of manually editing of the source code.
   #                          The config file is parsed, line by line, with which both SOURCE HOST VARIABLES and TARGET HOST VARIABLs are
   #                          populated.  This significantly streamlines the use of the script in any full backup scenario, without having
   #                          to edit any of the code within it. 
   #                        - also introduced a ./toBackup.txt file, which is used to list the paths to files and/or directories to 
   #                          backup.  This replaces the single-source version from before, with as many source files/directories
   #                          you can list in this txt file.
   #                        - also introduced optional script command arguments '--debug' and '--help', which gives the option of interactive
   #                          debugging and help menu respectively.
   #
  #               v1.2871  - OVERALL STATUS: WORKS VERY WELL, WITH v1.286 issue pending
   # 
   #                    1. v1.286 issue remains, ie.:
   ...
   #
   #                          2. ADDED & CONFIRMED WORKING:
   #                              2.1 Major changes to convert this script into *both* PUSH or PULL versions, where the addresses of the source
   #                              and target hosts determine whether data is PUSHed from a local host, or PULLed from a remote host.
   #                                  > in part added function fileOrDirExists () to check if a directory (remote or local) exists; refactored code to suite.
   #                              2.2 Major changes introduced to now backup files/directories listed in external ./toBackup.txt file, instead of only one 
   #                                  file/directory at any one time.  All output and error logging ammended to suite; transfered to targetDir on target host 
   #                                  (either local or remote) automatically.  This opens up the ability to streamline all future full backups with ease.
   #
   #
   #                          3. Introduced variable SERVER_IP:  future version to replace references to string "localhost" with "$SERVER_IP"
   #
  #               v1.286  - OVERAL STATUS: little kink crept in from prev. version. - process incorrectly kills itself
   #                           FUTURE IMPROVEMENTS                     RECOMMENDATION
   #                           1. rsync child process(es)              Instead of killing one or more child rsync processes - that another instance
   #                                                                   of this script might have started up - by finding those processes using pattern
   #                                                                   matching; we could instead just find and kill all subprocesses of the parallel
   #                                                                   (parent) instance instead.  BUT FOR NOW - this version seems to be working well.
   #
  #               v1.285  - OVERAL STATUS: a working release!
   #                       -   ISSUE                                   STATUS
   #                           ----------------------------------------------
   #                           1. troubleshooting v1.284.sh issue      FIXED!!
   #                           2. does 2nd instance of script kill     UPDATE:  the following is TBC.
   #                                                                   NO! **BUT** it does kill the 1st script's rsync subprocess, the 1st? and 
   #                                                                   therefore technically does work as it should!!  We might
   #                                                                   wish to tweak a bit more, to kill the parallel script prior
   #                                                                   to killing its rsync subprocess - BUT IT ISN'T REQUIRED. 
   #
  #				v1.284	- STATUS: testing... OUTCOME: innoculous code on line 611 (sourceAddress=....) terminates program alltogether!!??
   #				v1.283	- STATUS: testing...
   #
  #				v1.282	- STATUS: testing...
   #						- attending to regular expression which attempts to find previous rsync instance still running... if any
   #						- while-loop required to further sort kink in killing said instance
   #
  #				v1.281	- fixed auto pure log file issue (now correctly deleting oldest error & rsync logs)
   #
  #				v1.28	- auto purge (error, rsync) log files after default amount (30) - TEST PENDING
   #						- confirm source directory exists (for either local or remote host)
   #
  #				v1.27	- stable release (v1.261, w/ trial over)
   #
  #				v1.261	- TRIAL ONLY
   #							> new 'process ID' (PID) command to check for previous instance of this script already running (instead of using script getRsyncPID.sh)
   #						- ammended variable 'regex' for more accurate pattern matching:  
   #							> regex="rsync.*$sourceDir.*$targetDir"
   #                       
  #				v1.26	- automatic hostname assigned to SERVER_NAME (i.e. $HOSTNAME)
  #				v1.25	- first revision: a scaled down implementation of the incremental (snapshot) backup script 'incremental-backup_PULL-version_v1.25.sh'
   #
 #INSTRUCTIONS:	Very little required to tweak for different backup scenarios 
  ...
  #
  #                  c) you cannot backup between two remote hosts, only one of the following:
  #                          - local host to local host
  #                          - local host to remote host
  #                          - or remote host to local host
  # 
 #REQUIREMENTS:	
  #				1. System software:
...
##FUNCTIONS
 ...
 function printUsage () {  
    ...
    echo ""
    echo "  Configuring the backup script is done via ./full-backup.conf."
    echo ""
    echo "REQUIREMENTS:"
    echo "  1. Configuration files:     ./toBackup.txt, ./full-backup.conf"
...
##GENERAL VARIABLES
 ...
 backupConfig="$CURRENT_DIR/full-backup.conf"		#configuration file: file used to populate ALL script variables
...
##GET/SET SOURCE HOST VARIABLES - i.e. backup source
 # --------------------------------------------------
 # populate variables from config file (full-backup.config)
 # 
...
##PURGE OLDEST (ERROR AND RSYNC) LOG FILES BEFORE CONTINUING (when the max number logs is exceeded)
 ...
 if [[ $CURRENT_NUM_RSYNC_LOGS -ge $MAX_NUM_RSYNC_LOGS ]]; then
    ...
    if [[ "$targetAddress" == "localhost" ]]; then        
        ...
        if [[ $? != 0 ]]; then	        
          echo -e "\nERROR: MAX RSYNC LOG FILES EXCEEDED:  Oldest log file failed to delete.\nManual delete required." >> "$backupInfoDir/$ERROR_LOG"
	        echo -e "$EMAIL_MESSAGE" | mail -s "$EMAIL_SUBJECT" $EMAIL_RECEIVER
 	        #exit 1	
        fi
    else
 ...
 ##ITERATE OVER ARRAY OF SOURCE FILES/FOLDERS TO BACKUP AND EXECUTE RSYNC BACKUP:
  ...
  for fileOrDir in ${arrOfSrcFilesAndDirs[@]}; do       #IF ERROR OCCUR: try double quoting wrap around, ie. "${arrOfarrOfSrcFilesAndDirs[@]}"
    ...
    if [[ "$TEMP_EXIT_STATUS" != "0" ]]; then                     
        ...
        echo "  2. The target directory is empty when not expected to be (see ./full-backup.conf), " 2>&1 | tee -a "$backupInfoDir/$ERROR_LOG"
    ...
    # 2. from local source to local host:
        ...
        echo -e "\nBACKUP: source = localhost:$fileOrDir    target = localhost:$targetDir/" >> "$backupInfoDir/$RSYNC_LOG"
        echo    "-------------------------------------------------------------------------" >> "$backupInfoDir/$RSYNC_LOG"
    ...
    # 3. from local source to remote host (ie. data PUSHed to remote target)
        ...
        echo -e "\nBACKUP: source = localhost:$fileOrDir    target = $t_userName@$targetAddress:$targetDir/" >> "$backupInfoDir/$RSYNC_LOG"
        echo    "----------------------------------------------------------------------------------------" >> "$backupInfoDir/$RSYNC_LOG"       
    ...
    # 4. from remote host to remote host - THIS IS NOT ALLOWED! - terminate script w/ error message & log. 
    else
